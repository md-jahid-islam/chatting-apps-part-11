...
const Users = () => {
    ...
    onValue(starCountRef, (snapshot) => {
      ...
      snapshot.forEach((item) => {
        if (item.val().uid !== sliceUser.uid) {
          arr.push({ ...item.val(), key: item.key });  // Use item.key if it's valid here
        }
      });
      setUsers(arr);
    });
  ...
  const handleAdd = (friendData) => {
    // Pushing friend request to firebase
    ...
    // Update the requestSent state to track the sent request
      ...
      <div className="friend mt-[55px] flex flex-col items-center gap-4 ml-[350px] w-[400px] h-[500px] bg-[#077eff] rounded-lg shadow-2xl ">
        ...
        {users.map((item) => (
          <div key={item.key} className="friend-req bg-[#ffffff] rounded-lg w-[360px] h-[80px] flex items-center relative shadow-2xl">
            <img className=" w-[50px] h-[50px] rounded-full" src="public/img.png" alt="img" />
            ...
            >
              {requestSent === item.uid ? 'Request Sent' : 'Add Friend'}
            </button>
          </div>
        ))}
      </div>
